...
// Modificada por giltesa.com, para añadir compatibilidad con el IDE 1.x
// ademas de eliminar impresiones de informacion fuera del modo debugger.
...
class PN532
{
	public:
		...
		uint32_t readAllMemory(
			uint32_t cid,       /* Card NUID */
			uint8_t *data,      /* 1 to 752 bytes */
			uint32_t sizeData
		);

		uint32_t writeAllMemory(
			uint32_t cid,       /* Card NUID */
			uint8_t *data,      /* 1 to 752 bytes */
			uint32_t sizeData
		);
